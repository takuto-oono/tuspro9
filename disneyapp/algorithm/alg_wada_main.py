...
import datetime
from disneyapp.algorithm.alg_util import wrapper_alg
from disneyapp.algorithm.alg_wada import IPSO
...
def alg_main(date: datetime.date) -> Tuple[List[int], int]:
    ...
    try:
        expected_wait_time_data = s3.get_csv_file('expected_wait_time_data/expected_wait_time_data_{}.csv'.format(date.strftime('%Y%m%d')))
    except:
        print('except except')
        print(date.day)
